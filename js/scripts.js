	...
	function PlayVideo() {
		...
		if( $('.video-wrapper').length > 0 ){
			...
			$('.sound').on('click', function (e) {
				...
				// Certifique-se de que o vídeo foi encontrado
				if (video_object.length > 0) {
					video_object[0].muted = !video_object[0].muted; // Toggle muted
					$(this).toggleClass('muted'); // Alterna a classe CSS
					...
					// Impede que o evento afete elementos pais
					e.stopPropagation();
				} else {
...
---------------------------------------------------*/
	...
	const video = document.getElementById('myVideo');
        ...
        const soundText = muteButton.querySelector('.sound');

        muteButton.addEventListener('click', () => {
            video.muted = !video.muted;
            soundText.textContent = video.muted ? soundText.getAttribute('data-off') : soundText.getAttribute('data-on');
        });